...
def gensqlfromgrp(p_currdiff_block, p_updates_ids_list, p_keys_byref, p_limkeys_list, o_sql_src):	
		...
		for k in p_currdiff_block.keys():	
			...
			if isinstance(diff_item, list):
				...
				if "diffoper" in diff_item.keys():
					...
					gensqlfromgrp(diff_item, p_updates_ids_list, p_keys_byref+[k], p_limkeys_list, o_sql_src) 
					
	elif isinstance(p_currdiff_block, list):
		
		pass
		# gensqlfromgrp_list(p_currdiff_block, p_updates_ids_list) 
			
	return "OK"
	
				
def updatedb(p_proj, p_difdict, updates_ids_list, limkeys_list):
		...
		if grp in diff_content.keys():
			...
			if grp in ref_json.keys():
				...
				if grp in CFG_LISTGROUPS:
					...
					pass
					# gensqlfromgrp_list(currdiff_block, updates_ids_list)
								
				else:
					
					gensqlfromgrp(currdiff_block, updates_ids_list, [grp], limkeys_list, out_sql_src) 
